<?php
    ...
    //   $view = $this->db->get('onsite');
	  //   return $view->result();
		...
		function getOnSite(){
      $view = $this->db->query('SELECT * FROM `patient` p, onsite o WHERE p.status = "1" AND o.NIK = p.NIK');
	    return $view->result();
		}

		public function Counter()
		{
			$view = $this->db->query('SELECT `Number` from onsite order by Number DESC limit 1 ');
			return $view->result();
		}
